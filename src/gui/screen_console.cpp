...
Control placed(Control control, Slot slot, Menu menu=Menu::none) {
    ...
    if(slot==Slot::simulation)
        control.help="Select simulated transmit power and path attenuation. This determines actual channel C/N0 independently of the TX target settings. With the simulator's 10 dB noise figure, 3dBm -170dB gives -3 dB-Hz and 3dBm -120dB gives +47 dB-Hz. A lower TX target requests longer automatic integration; it does not improve the channel or remove clock/phase drift.";
    if(slot==Slot::simulation_confidence)
        control.help="Idealized whole-draft receive probability, conditional on finishing the receiver computation. This noise and independent-error model is not a measured success rate and does not predict whether simulation will finish promptly. Weak signals can require additional full searches when a bit is not accepted. Uses the selected waveform, exact message length and applicable error correction. Carrier outside RX search means the simulated frequency shift exceeds the finite search span; no numeric probability is supported there. Wide RX search exceeds RAM means the modeled memory allowance cannot hold the expanded search; a narrower runtime fallback has no modeled probability. Additional recovery is excluded.";
    ...
    if(slot==Slot::lpi_estimate) {
        ...
        control.help="Advisory energy-detector time and equivalent wire bits/symbols at 90% detection and 1% false alarm per known window. Assumes an unkeyed observer with the receiver's C/N0, known occupied band and on-air window, and stationary Gaussian noise of known power. Simulation uses its actual channel C/N0; otherwise the active draft's TX target is only an assumed C/N0, not a measurement. With encryption off, a warning identifies a hypothetical encrypted private pattern at the current sample, chip and symbol timing and C/N0. This includes tone experiments; actual public patterns and tones can be easier to detect and are not described by these figures. No key or waveform setting is changed. Numerical times apply only at or below -10 dB in-band SNR; the hypothetical warning remains when a number is unavailable. Detection can occur within one symbol and repeated traffic accumulates. Encryption does not reduce power or interference. No guaranteed hidden traffic or safe quota. Transmission details show the full model and current draft exposure including settling and suppression, without re-encoding for encryption.";
    }
...
const std::vector<Control>& console_screen() {
    static const std::vector<Control> controls{
        ...
        placed({Kind::label,Field::simulation_confidence,Command::none,Bitmap::none,Page::console,2,"RX success (noise model)"}, Slot::simulation_confidence),
