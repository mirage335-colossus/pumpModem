...
Control placed(Control control, Slot slot, Menu menu=Menu::none) {
    ...
    if(slot==Slot::simulation_confidence)
        control.help="Idealized receive probability for the draft, or one bit when the draft is empty, conditional on finishing the receiver computation. RX reference covers the original coherent match with the extra detector-choice penalty; added drift-tolerant gain is not yet estimated. This noise and independent-error model is not a measured success rate and does not predict whether simulation will finish promptly. Weak signals can require additional full searches when a bit is not accepted. Uses the selected waveform, exact message length and applicable error correction. Carrier outside RX search means the modeled frequency shift exceeds the finite search span; no numeric probability is supported there. Wide RX search exceeds RAM means the modeled memory allowance cannot hold the expanded search; a narrower runtime fallback has no modeled probability. Additional recovery is excluded.";
