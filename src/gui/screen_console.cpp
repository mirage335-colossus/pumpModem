...
Control placed(Control control, Slot slot, Menu menu=Menu::none) {
    ...
    if(slot==Slot::simulation_confidence)
        control.help="Estimated probability of receiving the entire current draft under the selected simulation channel and matching receiver settings. Uses an ideal noise and independent-error model with the selected waveform, exact message length and applicable error correction. Carrier outside RX search means the simulated frequency shift, including clock error at the carrier frequency, exceeds the requested finite search span; no numeric probability is supported there, even at high SNR. Wide RX search exceeds RAM means the modeled memory allowance cannot hold the expanded search. Reception may continue with a narrower local search, identified in live status, but no probability is modeled for that fallback. This is model-based probability, not measured receiver confidence or a guarantee. Additional recovery is excluded.";
    if(slot==Slot::simulation_cpu_time)
        control.help="Heuristic simulation computation time for a fixed high-end Intel Core i9-13900H laptop reference. Includes acquisition and serial tracking of one matching stream through complete-symbol absence; competing tracks can add work. No local benchmark or hardware detection. An estimate shorter than airtime does not guarantee live processing or prompt bit delivery. Replay playback and additional recovery time are excluded.";
