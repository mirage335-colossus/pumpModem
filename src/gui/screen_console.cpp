...
Control placed(Control control, Slot slot, Menu menu=Menu::none) {
    ...
    if(slot==Slot::simulation_confidence)
        control.help="Modeled receive probability for the draft, or one bit when the draft is empty, conditional on finishing receiver computation. Eligible long patterns include both coherent and four-section reception, with phase drift, shared noise and competing bit patterns. RX reference labels a limited fallback model. This is a statistical estimate, not a measured success rate or the probability of finishing promptly. It uses the selected waveform, exact message length and applicable error correction; additional recovery is excluded. Carrier outside RX search or Wide RX search exceeds RAM suppresses the percentage. The full adaptive receiver search and real-world interference are not simulated by this estimate.";
...
const std::vector<Control>& console_screen() {
    static const std::vector<Control> controls{
        ...
        placed({Kind::text,Field::planner_target,Command::none,Bitmap::none,Page::planner,3,"Planner target SNR (dB-Hz)",1,false,64}, Slot::planner_target),
