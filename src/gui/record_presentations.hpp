...
inline std::vector<ui::Record> signal_records(const Signals& signals) {
    ...
    for(std::size_t i=0;i<signals.lines().size();++i) {
        ...
        const auto tone=(signal.binary?signal.complete:signal.validated)?ui::TextTone::normal:ui::TextTone::muted;
        ui::Record row;row.id=std::to_string(signal.id);
        row.activatable=signals.copy_id(i).has_value()||signals.copy_bits(i).has_value();
        row.cells={
