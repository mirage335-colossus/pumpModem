...
Inspection inspect(const InspectionRequest& request) {
    ...
    result.lanes.push_back({"Receive • pattern evidence",{{"Bounded hypotheses","Search only the configured receive targets, selected rate, carrier and pattern mode, with local clock/key hypotheses."},
        ...
        {"Byte-boundary recovery",short_text || result.binary?"Raw bits and short dictionary text retain their exact bit lengths.":"After decryption, recognize complete or partially damaged alignment markers near their expected boundaries and restore byte grouping before Reed-Solomon correction. Bounded marker loss requires an intact trailing anchor and a unique endpoint. The ideal random-bit false-match bound is at most 2^-100 per recovery; packet integrity still determines acceptance. A recognized marker prevents failed packets from becoming dictionary text. Timing and keystream alignment remain controlled by pattern acquisition.",short_text || result.binary?InspectionState::off:InspectionState::active},
