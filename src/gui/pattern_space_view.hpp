...
public:
    ...
    int render(const inspection::PatternSpace& model,int x,int y,int width,bool paint) const {
        ...
        const auto paragraph=[&](const std::string& text,bool bold=false) {
            fl_font(bold?FL_HELVETICA_BOLD:FL_HELVETICA,12);
        ...
        if(paint) {
            ...
            for(std::size_t i=0;i<names.size();++i) {
                ...
                fl_color(b.enabled?fl_rgb_color(219,237,245):fl_rgb_color(237,241,244)); fl_rectf(b.x,b.y,b.w,b.h);
                fl_color(fl_rgb_color(196,213,224)); fl_rect(b.x,b.y,b.w,b.h);
                fl_font(FL_HELVETICA,11); fl_color(b.enabled?ink():muted());
                fl_draw(names[i],b.x,b.y,b.w,b.h,FL_ALIGN_CENTER);
            }
            fl_font(FL_HELVETICA_BOLD,12); fl_color(ink());
        ...
        if(paint) {
            fl_font(FL_HELVETICA,10); fl_color(muted()); fl_draw("Bits",x+14,grid_y-6);
            ...
            for(std::size_t row=0;row<model.coefficients.size();++row) {
                ...
                fl_color(ink()); fl_font(FL_COURIER,11); fl_draw(value.c_str(),x+14,top+13);
                for(std::size_t c=0;c<shown;++c) {
                    ...
                    fl_color(used?component(sample.real(),amplitude_scale):fl_rgb_color(213,217,221)); fl_rectf(left,top,cell_width-1,8);
                    fl_color(used?component(sample.imag(),amplitude_scale):fl_rgb_color(213,217,221)); fl_rectf(left,top+8,cell_width-1,8);
                }
            }
        }
        cursor+=19+static_cast<int>(model.coefficients.size())*row_height+8;
        paragraph("Blue: positive. Coral: negative. White: zero. Colour strength shows I/Q amplitude on one scale for all rows. Grey chips are outside this symbol's actual duration. Rows assume a zero preceding carrier phase.");
        ...
        const int map_size=map_cell*static_cast<int>(matrix_count),map_height=std::max(240,map_size),map_x=x+40,map_y=y+cursor+19;
        // Divide distances by sqrt(T) for the colour scale: a common factor
        ...
        if(paint) {
            for(std::size_t a=0;a<matrix_count;++a) {
                ...
                for(std::size_t b=0;b<matrix_count;++b) {
                    ...
                    fl_color(fl_rgb_color(static_cast<unsigned char>(22+210*fraction),
                                          static_cast<unsigned char>(55+178*fraction),
                                          static_cast<unsigned char>(79+160*fraction)));
                    fl_rectf(map_x+static_cast<int>(b)*map_cell,map_y+static_cast<int>(a)*map_cell,map_cell,map_cell);
                }
            }
            fl_font(FL_HELVETICA,12); fl_color(ink());
            ...
            fl_draw(detail.c_str(),map_x+map_size+28,map_y,width-map_size-90,map_height,
                    FL_ALIGN_LEFT|FL_ALIGN_TOP|FL_ALIGN_INSIDE|FL_ALIGN_WRAP,nullptr,0);
        }
        ...
        const auto evidence=[&](const std::string& name,double correlation,double residual) {
            ...
            if(paint) {
                fl_font(FL_HELVETICA,11); fl_color(ink());
                fl_draw(name.c_str(),x+14,y+cursor+17);
                fl_color(fl_rgb_color(231,238,243)); fl_rectf(bar_x,y+cursor+5,bar_width,16);
                const auto fraction=std::clamp(1-residual,0.,1.);
                const int matched=static_cast<int>(std::lround(fraction*bar_width));
                fl_color(fl_rgb_color(25,139,164)); fl_rectf(bar_x,y+cursor+5,matched,16);
                fl_color(fl_rgb_color(231,175,139)); fl_rectf(bar_x+matched,y+cursor+5,bar_width-matched,16);
                fl_color(muted());
        ...
        paragraph("Blue bar: energy matching the code. Coral: energy outside that code. For noise, rho is RMS over independent whitened chip coordinates; individual noise realizations vary.");
...
private:
    ...
    static Fl_Color ink() { return fl_rgb_color(25,45,60); }
    static Fl_Color muted() { return fl_rgb_color(79,100,115); }
    static std::string number(double value) { std::ostringstream out; out<<std::setprecision(3)<<value; return out.str(); }
    static Fl_Color component(double value,double scale) {
        const auto fraction=scale>0?std::clamp(std::abs(value)/scale,0.,1.):0;
        const std::array<int,3> target=value>=0?std::array<int,3>{20,117,164}:std::array<int,3>{205,79,49};
        std::array<unsigned char,3> color{};
        for(std::size_t i=0;i<color.size();++i) color[i]=static_cast<unsigned char>(std::lround(250+(target[i]-250)*fraction));
