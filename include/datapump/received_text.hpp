...
// Line-feed newlines are allowed in both modes. Callers may enable additional
// printable ASCII only while both Developer and Shellcode modes are enabled.
// Raw received bytes remain separate for explicit saving.
inline std::string received_text(std::span<const std::uint8_t> bytes,bool shellcode=false) {
    ...
    for(std::size_t i=0;i<bytes.size();++i) {
        ...
        const bool allowed=byte=='\n' || (shellcode ? byte>=0x20 && byte<=0x7e :
