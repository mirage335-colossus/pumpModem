...
int main(){try {
    ...
    for(const std::size_t count:{1,3,5,7,31}) {
        ...
        require(samples.size()==m::training_sample_count(config)+count*8*m::symbol_sample_count(config),"byte modulation sample count mismatch");
    ...
    require(wave.size()==m::training_sample_count(config)+status.size()*m::symbol_sample_count(config),"raw bits were padded");
