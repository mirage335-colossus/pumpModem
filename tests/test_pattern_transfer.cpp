...
transfer::Options options(bool keyed=false) {
    ...
    result.modem=tuning::resolve(1200,40,keyed?tuning::PatternMode::auto_keystream:tuning::PatternMode::auto_pattern,keyed).config;
    return result;
}
void exact_short_text() {
    ...
    for(bool keyed:{false,true}) {
        ...
        for(auto fec:{FecMode::off,FecMode::rs20,FecMode::rs60}) {
            ...
            check(estimate.waveform_samples==3*modem::symbol_sample_count(value.modem),"short text has no preamble/header/checksum/padding/FEC");
...
void packet_downstream() {
    ...
    check(source->total_samples()==bits.size()*modem::symbol_sample_count(value.modem),"packet bit count remains exact at the content limit");
...
int main(){try{exact_short_text();raw_bits();packet_downstream();public_late_symbol_interpretation();std::cout<<"pattern transfer tests passed\n";return 0;}catch(const std::exception& e){std::cerr<<e.what()<<'\n';return 1;}}
