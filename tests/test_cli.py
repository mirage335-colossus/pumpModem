...
class StreamCLI(unittest.TestCase):
    def run_pump(self,*args,data=None,ok=True,timeout=90):
        ...
        return p
    def run_pump_with_exact_save(self,*args,source,data=None,receive_options=AUDIO):
        # Preserve byte-for-byte transport assertions independently of the
        # intentionally lossy received-text presentation. simulate can write
        # its sampled channel capture; only rx supports saving decoded bytes.
        with tempfile.TemporaryDirectory() as directory:
            destination=pathlib.Path(directory)/'received.bin'
            if args[0]=='simulate':
                if '--output' in args:
                    capture=args[args.index('--output')+1]
                    output=()
                else:
                    capture=pathlib.Path(directory)/'simulated.wav'
                    output=('--output',capture)
                result=self.run_pump(*args,*output,data=data)
                self.run_pump('rx','--input',capture,'--save',destination,*receive_options)
            else:
                self.assertEqual(args[0],'rx')
                result=self.run_pump(*args,'--save',destination,data=data)
    ...
    def test_short_dictionary_text_received_with_exact_bits(self):
        source=b'e\x00'
        value=json.loads(self.run_pump_with_exact_save('simulate','--input','-','--json','--snr','30',
            '--clock-error-ppm','0','--phase-noise','0',*AUDIO,data=source,source=source).stdout)
    ...
    def test_received_text_is_restricted_in_json_and_pipes(self):
        ...
        for source in sources:
            ...
            self.assertEqual(value['data_text'],safe_received(source))
            self.assertNotIn('data_base64',value)
            self.assertLessEqual(set(value['raw_bits']),set('01'))
            piped=self.run_pump(*args,data=source).stdout
            self.assertEqual(piped,safe_received(source).encode('ascii'))
    def test_received_attachment_name_is_restricted(self):
        with tempfile.TemporaryDirectory() as directory:
            source=pathlib.Path(directory)/'payload.bin'
            source.write_bytes(b'\x00\xff;payload')
            name='payload;(x)&caf\u00e9.bin'
            options=('simulate','--input',source,'--filename',name,'--snr','30',
                     '--clock-error-ppm','0','--phase-noise','0',*AUDIO)
            value=json.loads(self.run_pump(*options,'--json').stdout)
            self.assertEqual(value['filename'],safe_received(name.encode('utf-8')))
            human=self.run_pump(*options)
            self.assertIn(safe_received(name.encode('utf-8')).encode('ascii'),human.stderr)
            self.assertNotIn(name.encode('utf-8'),human.stderr)
    ...
    def test_link_analysis_exact_draft_and_airtime(self):
        ...
        for draft,count in ((('--bits','0'),1),(('--bits','001'),3),(('--text','a'),3)):
            ...
            self.assertGreaterEqual(value['current_receiver']['payload_processing_seconds'],value['current_receiver']['mitigation_seconds'])
            self.assertGreaterEqual(value['current_receiver']['receiver_cpu_seconds'],value['current_receiver']['payload_processing_seconds'])
            self.assertGreaterEqual(value['current_receiver']['cpu_seconds'],value['current_receiver']['receiver_cpu_seconds'])
    ...
    def test_sub_hertz_sampled_reception(self):
        ...
        self.assertEqual(value['data_text'],'a')
    ...
    def test_automatic_profile_short_text_and_binary_dictionary(self):
        ...
        for source,expected_bits in (('quick brown',70),('quick brown fox ',98),('e'*16,48)):
            ...
            self.assertEqual(value['data_text'],source)
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(value['data_text'],'t')
            self.assertTrue(value['short_text_decoded'])
    def test_three_bit_dictionary_text_and_physical_end(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertFalse(value['content_validated'])
            self.assertEqual(value['raw_bits'],'001')
            self.assertEqual(value['data_text'],'e')
            ...
            self.assertEqual(pending['data_text'],'')
    ...
    def test_uncompressed_exact_binary(self):
        ...
        value=json.loads(self.run_pump_with_exact_save('simulate','--input','-','--no-compression','--json',
            '--snr','30','--clock-error-ppm','0','--phase-noise','0',*AUDIO,data=source,source=source,
            receive_options=(*AUDIO,'--no-compression')).stdout)
        self.assertTrue(value['content_validated'])
        self.assertEqual(value['data_text'],safe_received(source))
        self.assertEqual(value['kind'],'text')
        self.assertEqual(value['filename'],'')
    def test_explicit_attachment(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            value=json.loads(self.run_pump_with_exact_save('simulate','--input',path,'--repeatable','--json',
                '--snr','30','--clock-error-ppm','0','--phase-noise','0',*AUDIO,source=source).stdout)
            self.assertTrue(value['content_validated'])
    ...
    def test_wav_end_and_eof_are_distinct(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(incomplete['data_text'],'')
    ...
    def test_simulate_output_retains_observed_absence(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            simulated=json.loads(self.run_pump_with_exact_save('simulate','--input','-','--output',path,'--json',
                '--snr','30','--clock-error-ppm','0','--phase-noise','0',*AUDIO,data=source,source=source).stdout)
            recorded=json.loads(self.run_pump_with_exact_save('rx','--input',path,'--json',*AUDIO,source=source).stdout)
