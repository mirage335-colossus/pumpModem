...
namespace {
constexpr double reference_snr_db=3;
...
Profile short_profile() {
    ...
    require(preset.reference_bandwidth_hz==reference_bandwidth_hz&&
        default_expected_snr(Channel::acoustic_short)==reference_snr_db,
        "short acoustic default lost its original-band 3 dB reference");
    require(p.channel==Channel::acoustic_short&&p.acoustic_ofdm&&p.capacity_mode&&
        p.ldpc_frame_bits==16200&&p.ofdm_training_blocks<16&&p.interleave_depth==1,
        "short acoustic regression no longer uses the separate short-frame profile");
    require(p.sample_rate==48000&&p.ofdm_prefix_samples>echo_delay_samples,
...
double noise_sigma(const Profile& p) {
    ...
    const auto nominal_power=std::pow(p.amplitude/4.5,2);
...
struct Fixture {
    ...
    Fixture(bool encrypted,bool attachment,bool short_text=false):
        ...
        if(source.size()==target_source_bytes)
        ...
        Transmitter tx(profile,[&](std::span<std::uint8_t> bits){return encoder.next_interval(bits);});
...
void sampled_completion(const char* label,bool encrypted,bool attachment,bool short_text,
    ...
    require(intervals==f.estimate.intervals&&state.intervals==f.estimate.intervals&&
        !state.failed_cycles&&!state.ldpc_failed_frames&&state.ldpc_changed_bits>0&&
        state.ldpc_frames==f.estimate.intervals/cycle_intervals(f.profile)*f.profile.interleave_depth,
        "short acoustic noisy coding fixture lost fixed positions or did not exercise LDPC correction");
...
void incomplete_controls() {
    ...
    const auto block=f.profile.ofdm_fft_size+f.profile.ofdm_prefix_samples;
    const auto prefix=(preamble_symbols(f.profile)-1)*block;
    ...
    require(!truncated.progress().acquired&&!truncated.progress().physical_complete&&!intervals,
        "short acoustic incomplete training was accepted without its independent final marker");
