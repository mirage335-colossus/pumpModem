...
struct Receiver::Impl {
    ...
    Impl(Profile p,IntervalSink target,SymbolObserver observe,SymbolObserver observe_input):config(p),sink(std::move(target)),observer(std::move(observe)),
        ...
        filtered.resize(static_cast<std::size_t>(std::ceil((sync_symbols+32+(p.capacity_mode?training_symbols:0))*sps))+64);
    }
    ...
    CapacityFit capacity_training_fit(double end,double period) const {
        constexpr std::size_t count=training_symbols+sync_symbols-16;
        std::array<Complex,count> despread{};
        Complex first=0,last=0;
        double energy=0;
        for(std::size_t j=0;j<count;++j) {
            const auto k=j+8;
            const auto expected=k<training_symbols?marker[(k*17+9)%sync_symbols]*Complex(0,1):marker[k-training_symbols];
            const auto age=static_cast<double>(training_symbols+sync_symbols-1-k);
        ...
        for(unsigned pass=0;pass<4;++pass) {
            for(unsigned dimension=0;dimension<2;++dimension) {
                ...
                const auto score=[&](double candidate) {
                    constexpr double center_age=(training_symbols+sync_symbols-1)*.5;
                ...
                if(dimension) {
                    ...
                    end+=(training_symbols+sync_symbols-1)*.5*(best-period);
                    period=best;
                }
                else end=best;
            }
