...
int main(int argc,char**argv) {
 if(argc!=5)return 2;
 const std::string name=argv[1]; const auto rate=name=="3/4"?CodeRate::three_quarters:name=="8/9"?CodeRate::eight_ninths:CodeRate::nine_tenths;
 ...
 auto p=capacity_profile();p.channel=Channel::acoustic;p.acoustic_ofdm=true;p.constellation=16;p.code_rate=rate;p.interleave_depth=8;
 const auto k=ldpc::data_bits(rate);const auto points=square_qam_constellation(16);
 ...
 for(unsigned cycle=0;cycle<cycles;++cycle) {
  ...
  for(std::size_t s=0;s<bits.size();s+=4) {
   unsigned label=0;for(unsigned j=0;j<4;++j)label=(label<<1)|bits[s+j];
   const auto received=points[label]+std::complex<double>(sigma*normal(rng),sigma*normal(rng));
   square_qam_soft_demodulate(16,received,metric);
 ...
 std::cout<<std::setprecision(10)<<name<<",16,"<<db<<","<<frames<<","<<seed<<","<<failed<<","<<nonconverged<<","<<undetected<<","<<wrong<<","<<double(raw)/(frames*ldpc::coded_bits)<<","<<1-loss/(std::log(2.)*frames*ldpc::coded_bits)<<","<<double(iterations)/frames<<","<<seconds<<"\n";
}
