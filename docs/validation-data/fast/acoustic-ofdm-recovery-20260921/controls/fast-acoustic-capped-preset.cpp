...
double capped_default_expected_snr(Channel c) {
    switch(c) {
...
std::vector<double> capped_expected_snr_options(Channel c) {
    const auto top=capped_default_expected_snr(c),bottom=top-40;
...
SnrPreset capped_resolve_snr_preset(Channel c,double expected) {
    const auto nominal=capped_default_expected_snr(c),band=reference_bandwidth(c);
    ...
    for(const bool ofdm:{true,false}) {
        ...
        for(unsigned order=4;order<=default_profile.constellation;order*=4)for(auto rate:rates) {
            ...
            if(ofdm) {
                ...
                // Cap is applied after validity below.
            } else {
            ...
            if(!valid(p))continue;
            if(ofdm) {
                const auto max_blocks=total_interval_symbols(default_profile,cycle_intervals(default_profile))+1;
                while(p.interleave_depth>1 && total_interval_symbols(p,cycle_intervals(p))+1>max_blocks)
...
std::string capped_symbol_rate_option_id(const Profile& p) {
    validate(p);
    return p.acoustic_ofdm?"ofdm:"+std::to_string(p.ofdm_fft_size):
        "sc:"+number(p.symbol_rate,std::numeric_limits<double>::max_digits10);
}
std::vector<SymbolRateOption> capped_symbol_rate_options(const Profile& p) {
    validate(p);
    ...
    const auto add=[&](Profile q) {
        if(!valid(q))return;
        if(std::none_of(choices.begin(),choices.end(),[&](const auto& old){return capped_symbol_rate_option_id(old)==capped_symbol_rate_option_id(q);}))choices.push_back(q);
    };
    add(p);
    if(p.acoustic_ofdm) {
    ...
    for(const auto& q:choices) {
        ...
        out.push_back({capped_symbol_rate_option_id(q),number(baud)+
            (q.acoustic_ofdm?" /s/tone · "+number(1000./baud)+" ms":" symbols/s")});
    }
    return out;
}
Profile capped_apply_symbol_rate_option(Profile p,std::string_view id) {
    validate(p);
    if(id=="auto") {
