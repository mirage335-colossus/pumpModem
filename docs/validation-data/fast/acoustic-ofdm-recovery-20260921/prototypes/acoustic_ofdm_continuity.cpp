...
struct Receiver::Impl {
    ...
    void process_block() {
        ...
        const bool continuity=present||(std::abs(common)>.1&&std::abs(common)<10&&verify(y,block,signature,common,delay,32));
        ++state.symbols;
        if(continuity)absent=0;else absent+=g.length*period/p.sample_rate;
        ...
        if(present)state.evm=std::sqrt(error/std::max<std::size_t>(count,1));
        if(continuity) {
