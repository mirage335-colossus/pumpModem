...
constexpr std::size_t training_blocks=3;
...
std::uint64_t transmission_samples(const Profile& p,std::size_t intervals) {
    ...
    const auto blocks=checked_product(intervals/g.cycle_intervals,g.blocks);
...
double gross_bitrate(const Profile& p){Geometry g(p);return static_cast<double>(g.cycle_bits)*p.sample_rate/(g.blocks*g.length);}
...
struct Transmitter::Impl {
    ...
    std::uint64_t block=0,samples=0;bool done=false,have_cycle=false;
    Impl(Profile config,IntervalReader reader,SymbolObserver cb):p(config),g(config),source(std::move(reader)),observer(std::move(cb)),bits(g.cycle_bits) {
    ...
    bool load_cycle() {
        ...
        have_cycle=true;block_in_cycle=0;return true;
    }
    bool next() {
        if(!have_cycle&&!load_cycle()){done=true;return false;}
        std::vector<C> spectrum(g.n);
        if(block<training_blocks) {
            for(const auto k:g.active)spectrum[k]=known(k,block<2?0:2);
        } else {
...
struct Receiver::Impl {
    ...
    std::vector<float> pcm,cycle_soft,pending_soft;std::uint64_t first=0,received=0,next_search=0,block=3;
    std::vector<C> reference_fft,channel;std::vector<double> variance;
    std::vector<std::size_t> signature;
    bool eof=false,pending=false,locked=false;
    double candidate=0,next_window=0,period=1,absent=0,reference_energy=0;
    std::size_t block_in_cycle=0,soft_count=0;
    Impl(Profile config,IntervalSink target,SymbolObserver cb,SymbolObserver input,DiagnosticObserver diag):p(config),g(config),sink(std::move(target)),observer(std::move(cb)),input_observer(std::move(input)),diagnostic(std::move(diag)),
    ...
    double sample_at(double time) const {
        ...
        for(int j=-15;j<=16;++j) {
            ...
            const auto weight=(std::abs(t)<1e-12?1.:std::sin(pi*t)/(pi*t))*(.42+.5*std::cos(pi*t/16)+.08*std::cos(2*pi*t/16));
    ...
    bool verify(const std::vector<C>& observed,std::uint64_t ordinal,std::span<const std::size_t> bins,C gain=1.,double delay=0) const {
        ...
        for(auto k:bins) {
            ...
            const auto point=observed[k]*std::conj(expected);
    ...
    void acquire() {
        ...
        a=spectrum(candidate,period);b=spectrum(candidate+g.length*period,period);
        std::vector<double> raw_variance(g.n);
        for(auto k:g.active) {
            channel[k]=(a[k]+b[k])*.5/known(k,0);
            raw_variance[k]=std::norm(a[k]-b[k])*.5;
        }
        for(auto k:g.active) {
        ...
        auto verification=spectrum(candidate+2*g.length*period,period);
        if(!verify(verification,2,check)){pending=false;return;}
    ...
    void process_block() {
        ...
        double pilot_error=0,pilot_power=0;
        for(auto k:g.track) {
            const auto expected=channel[k]*known(k,block)*common*std::polar(1.,-2*pi*k*delay/g.n);
            pilot_error+=std::norm(y[k]-expected);pilot_power+=std::norm(expected);
        }
        ...
        for(auto k:g.data) {
            ...
            const auto nv=std::max(variance[k]*std::norm(common)/std::max(norm,1e-30),pilot_error/std::max(pilot_power,1e-30));
            if(diagnostic)try{diagnostic({block,(block-training_blocks)/g.blocks*g.cycle_bits+offset,k,value,nv,norm,state.clock_error_ppm,delay,present});}catch(...){}
            unsigned closest=0;
        ...
        if(block_in_cycle==g.blocks){block_in_cycle=0;soft_count=0;std::fill(cycle_soft.begin(),cycle_soft.end(),0);}
    }
    void ingest(std::span<const float> values) {
        ...
        for(;;) {
            ...
            if(locked) {
                ...
                if(received<candidate+3*g.length*1.001+20)break;
                acquire();
            } else {
