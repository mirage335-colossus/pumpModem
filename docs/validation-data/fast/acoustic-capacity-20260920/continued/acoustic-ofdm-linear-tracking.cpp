...
struct Receiver::Impl {
    ...
    void process_block() {
        ...
        std::vector<C> corrected(g.n), residual(g.n);
        for(auto k:g.track)residual[k]=y[k]/known(k,block)-channel[k]*common*std::polar(1.,-2*pi*k*delay/g.n);
        for(auto k:g.active) {
            auto it=std::lower_bound(g.track.begin(),g.track.end(),k);
            auto hi=it==g.track.end()?g.track.back():*it;
            auto lo=it==g.track.begin()?hi:*(it-1);
            auto t=hi==lo?0.:double(k-lo)/(hi-lo);
            corrected[k]=channel[k]*common*std::polar(1.,-2*pi*k*delay/g.n)+.8*((1-t)*residual[lo]+t*residual[hi]);
        }
        ...
        std::vector<double> pilot_variance;pilot_variance.reserve(g.verify.size());
        for(auto k:g.verify) {
            const auto expected=corrected[k]*known(k,block);
        ...
        for(auto k:g.data) {
            ...
            const auto nearest=static_cast<std::size_t>(std::lower_bound(g.verify.begin(),g.verify.end(),k)-g.verify.begin());
            double local_error=0;std::size_t local_count=0;
            for(auto j=nearest>2?nearest-2:0;j<std::min(g.verify.size(),nearest+3);++j) {
