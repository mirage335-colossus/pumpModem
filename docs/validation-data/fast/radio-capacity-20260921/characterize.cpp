...
void sampled_radio(Channel channel,double snr_db,unsigned qam,CodeRate rate) {
    ...
    p.interleave_depth=1;p.constellation=qam;p.code_rate=rate;
    ...
    std::cout<<channel_name(channel)<<" input_snr_db="<<snr_db<<" complete="<<state.complete
        <<" qam="<<qam<<" rate="<<code_rate_name(rate)<<" intervals="<<state.intervals<<" ldpc_frames="<<state.ldpc_frames<<" erased_intervals="<<rx.progress().erased_intervals<<" ldpc_failed="<<state.ldpc_failed_frames
...
int main() {try {
    sampled_radio(Channel::ssb,17,64,CodeRate::three_quarters);
    sampled_radio(Channel::ssb,18,64,CodeRate::three_quarters);
    sampled_radio(Channel::ssb,20,64,CodeRate::eight_ninths);
    sampled_radio(Channel::ssb,23,256,CodeRate::two_thirds);
